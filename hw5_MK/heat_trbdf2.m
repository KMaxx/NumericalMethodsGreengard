function [h,k,error] = heat_trbf2(m)
%
% heat_CN.m
%
...
nsteps = round(tfinal / (2*k));    % number of time steps
...
for n = 1:nsteps
     ...
     if rem(n,2)==1 %Trapezoidal for n+1/2
         g0np = (g0n+utrue(ax,tnp+k/2))/2;
         g1np = (g1n+utrue(bx,tnp+k/2))/2;
         ...
         % solve linear system:
         uint = A1\rhs;
     else  %Backward Difference Formula of 2nd order for n+1 
         uint = u(2:(m+1));   % interior points (unknowns)
         rhs = (-upre+4*uint)/3;
